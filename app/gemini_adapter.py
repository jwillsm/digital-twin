"""
Gemini adapter: Google Generative Language API with resource optimization for low-end hardware.

Optimizations for 7th Gen i3 + 4GB RAM + HDD:
- Class-based lazy initialization
- Reduced context windows and token limits
- Efficient REST calls using urllib
...
class GeminiAdapter:
    """Gemini LLM adapter with resource optimization."""
    ...
    def _call_gemini_chat(
        ...
        """Call Gemini API via REST with error handling."""
        self._ensure_initialized()
        endpoint = f"https://generativelanguage.googleapis.com/v1beta2/{self.model}:generate"
        headers = {
        ...
        try:
            ...
            logger.error(f"Gemini API error: {err_data}")
            raise
    ...
    def query(
        ...
        try:
            ...
            answer = self._call_gemini_chat(system_prompt, user_prompt, max_tokens=QUERY_MAX_TOKENS)
            return f"*{agent_name}*\n\n{answer}"
        except Exception as e:
...
**⚡ Priority**"""
            ...
            return self._call_gemini_chat(ALL_PILLARS_PERSONA, prompt, max_tokens=BRIEF_MAX_TOKENS)
        except Exception as e:
...
def query_gemini(user_question: str, pillar: Optional[str] = None, n_results: int = CONTEXT_LIMIT) -> str:
    """Legacy function wrapper."""
    from app.provider import ProviderFactory
    return ProviderFactory.get_provider().query(user_question, pillar, n_results)


def brief_gemini(limit: int = 20) -> str:
    """Legacy function wrapper."""
    from app.provider import ProviderFactory
